...
for slots. That allows:

| Layout | Slots | Slot size | Fits |
| --- | --- | --- | --- |
| `multiboot-2slot.csv` (default) | 2 | 6 MiB | two full size firmwares |
| `multiboot-3slot.csv` | 3 | 4 MiB | three medium firmwares (CrossPoint does **not** fit) |
| `multiboot-5slot.csv` | 5 | ~2.4 MiB | five small firmwares (TRMNL sized) |
...
- The layouts above are generated by `tools/gen_partitions.py`, so a different
  trade-off is one command away.
- Firmwares can also be kept on the **SD card** under
  ...
  not currently in a slot takes a few seconds while the image is copied.

> **Q1.** Is "2–3 firmwares resident in flash, unlimited firmwares on the SD
> card" an acceptable reading of "support for many simultaneous firmwares"? If
> not, the only alternative is restricting the project to small firmwares.
...
- Installing the selector **erases the whole device** (books, settings, WiFi
  credentials). The installer offers a full backup first.
...
> **Q2.** Do you accept the "erase everything, replace bootloader and partition
> table" install, or should the project instead try to fit into the existing
> CrossPoint partition table (two slots, no factory partition, sticky boot only)?
...
> **Q3.** Should slots share one data area (current behaviour, maximum space) or
> should a future layout give each slot a small private NVS at the cost of slot
> size?
...
> **Q4.** Is "disable auto-update inside guest firmwares, or expect the slot to
> be overwritten" acceptable, or do you want the installer to warn/patch?
